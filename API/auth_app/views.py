...
class AdminViewSet(viewsets.ViewSet):
    ...
    def set_password(self, request):
        ...
        cmd = "echo '{0}:{1}\' | chpasswd".format(request.user.username, serializer.data['new_password'])
        s, o = sudo_runner(cmd)
        if not s:
            log('security', 'admin', 'change_password', 'fail',
                username=request.user.username, ip=get_client_ip(request))
            raise serializers.ValidationError('Cannot set new password due to this error {}'.format(o))

        log('security', 'admin', 'change_password', 'success',
            username=request.user.username, ip=get_client_ip(request))
